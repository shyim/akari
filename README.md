...
<p align="center">
  <em>A high-performance PHP observability extension.<br>Automatic tracing, sampling, and profiling — exported as OpenTelemetry.</em>
</p>
...
Akari (灯, "light") is a PHP C extension that illuminates your application's behavior. It hooks into the Zend Engine via the official `zend_observer` API to capture every function call, database query, HTTP request, and exception — all exported as standard OpenTelemetry traces to any OTLP-compatible collector (Jaeger, Grafana Tempo, Honeycomb, etc.).
...
akari.trace_functions=1     ; trace all user functions
```
...
| `akari.mode` | `trace` | `trace` (every call) or `sample` (periodic timer) |
| `akari.max_depth` | `64` | Max call stack depth (1–256, clamped) |
| `akari.trace_internal` | `0` | Trace PHP built-in functions (high overhead) |
| `akari.trace_functions` | `0` | Trace all user functions, not just registered hooks |
| `akari.sample_period` | `0.01` | Sampling interval in seconds (sample mode) |
...
**Production — I/O only (minimum overhead):**
```ini
akari.enable=1
akari.service_name=app-prod
akari.mode=auto              ; trace mode, only I/O + framework hooks
...
**Sampling (lowest overhead):**
```ini
akari.enable=1
akari.mode=sample
akari.sample_period=0.01     ; 10ms sampling interval
```
...
├── hook_sampler.c                # Timer-based sampling
...
├── sampler_timer.h               # Platform timer abstraction
├── sampler_timer_kqueue.c        # macOS kqueue timer
└── sampler_timer_posix.c         # Linux POSIX timer
forwarder/
├── cmd/akari-forwarder/           # Go entry point
└── internal/                     # Receiver, buffer, transform, forwarder
tests/
└── *.phpt                        # 74 PHPT integration tests
```
...
```bash
make test                          # Run all 74 PHPT tests
