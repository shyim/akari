                               ...
                               profiler_span_t *span, uint32_t span_index)
{
    ...
    if (num_args >= 2) {
        ...
        if (src && dst && Z_TYPE_P(src) == IS_STRING && Z_TYPE_P(dst) == IS_STRING) {
            ...
            size_t buf_len = snprintf(buf, sizeof(buf), "%.*s -> %.*s",
                (int)Z_STRLEN_P(src), Z_STRVAL_P(src),
                (int)Z_STRLEN_P(dst), Z_STRVAL_P(dst));
            if (buf_len >= PROFILER_HTTP_URL_MAX) buf_len = PROFILER_HTTP_URL_MAX - 1;
                       ...
                       profiler_span_t *span, uint32_t span_index)
{
    ...
    if (num_args >= 1) {
        ...
        if (key_arg && Z_TYPE_P(key_arg) == IS_STRING) {
            char buf[256];
            size_t buf_len = snprintf(buf, sizeof(buf), "%s %s", fname, Z_STRVAL_P(key_arg));
