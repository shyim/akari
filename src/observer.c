...
static zend_observer_fcall_handlers observer_init(zend_execute_data *execute_data)
{
    ...
    if (!state || !state->active || state->mode != PROFILER_MODE_TRACE) {
...
static void observer_fcall_begin(zend_execute_data *execute_data)
{
    ...
    if (!state || !state->active || state->stack_depth >= state->max_depth
        ...
        if (state && state->active) {
            state->stack[state->stack_depth++] = (size_t)-1;
        }
        return;
    }
    ...
    /* Filtering: skip unregistered function calls */
    if (is_userland) {
        if (!state->trace_functions && !hook) {
            /* Push sentinel — fcall_end will still fire and needs to pop */
            state->stack[state->stack_depth++] = (size_t)-1;
            return;
        }
    } else {
        if (!hook && !state->trace_internal) {
            state->stack[state->stack_depth++] = (size_t)-1;
            return;
        }
    }
    ...
    state->stack[state->stack_depth++] = span_idx;
}
...
static void observer_fcall_end(zend_execute_data *execute_data, zval *return_value)
{
    ...
    if (!state || !state->active || state->stack_depth == 0) return;
