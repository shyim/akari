...
static void send_datagram(const uint8_t *data, size_t len)
{
    if (g_udp_fd < 0 || len == 0 || len > 65000) return;
...
void udp_export_spans(profiler_state_t *state, const char *service_name)
{
    ...
    size_t total_spans = state->span_count + (has_root ? 1 : 0);
    ...
    for (size_t i = 0; i < state->span_count; i++) {
        write_span_msgpack(&buf, state, &state->spans[i], i);

        if (buf.len > 60000 && i + 1 < state->span_count) {
            send_datagram(buf.data, buf.len);

            buf.len = 0;
            size_t remaining = state->span_count - (i + 1);
            msgpack_write_map(&buf, 4);
            msgpack_write_key(&buf, "v");
            msgpack_write_uint8(&buf, 1);
            msgpack_write_key(&buf, "sn");
            ...
            msgpack_write_bin(&buf, state->trace_id, 32);
            msgpack_write_key(&buf, "sp");
            msgpack_write_array(&buf, (uint32_t)remaining);
        }
    }
